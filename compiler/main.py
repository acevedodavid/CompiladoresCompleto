...
def p_asignacion(p):
    '''asignacion : ID pn_push_operand_and_type ASSIGN pn_push_operator expresion pn_assign SEMICOLON'''
    #print("termino asgignacion")
    #ID pn_push_operand_and_type dimensiones ASSIGN pn_push_operator expresion pn_assign SEMICOLON
...
def p_llamada(p):
    'llamada : ID pn_verify_func LPAREN pn_func_era aux_llamada pn_arg_count RPAREN pn_func_gosub'
...
def nextCteAddress(var_type):
    global const_b # constant base
    global int_b, float_b, char_b, mem_size # type bases
    global cte_int_counter, cte_float_counter, cte_char_counter

    if (cte_int_counter >= mem_size or cte_float_counter >= mem_size or cte_char_counter >= mem_size):
        print("Error, no hay espacio de memoria suficiente")
        sys.exit()
    ...
    if (var_type == 'int'):
        result += int_b
        result += cte_int_counter
        cte_int_counter += 1
    elif (var_type == 'float'):
        result += float_b
        result += cte_float_counter
        cte_float_counter += 1
    elif (var_type == 'char'):
        result += char_b
        result += cte_char_counter
        cte_char_counter += 1
    else:
        print("Error: se recibio una variable de un tipo no valido")
        sys.exit()
...
def p_pn_add_function(p):
    ...
    global int_b, float_b, char_b, bool_b
    current_param_count = 0
    current_func_var_count = 0
...
def p_pn_param_count(p):
    'pn_param_count : '
    global symbols, current_function, current_param_count
    symbols[current_function]['param_count'] = current_param_count

def p_pn_arg_count(p):
    'pn_arg_count : '
    #print("pn_arg_count")
    ...
    if (symbols[current_function_call]['param_count'] == current_argument_count):
        ...
        print(current_function_call + " expects " + symbols[current_function_call]['param_count'] + " arguments.\n")
        sys.exit()
    #print("salida")
...
def p_pn_verify_argument(p):
    ...
    global current_param_count, symbols, quadruples, current_function_call, current_argument_count
    ...
    if (current_argument_count <= len(param_list)):
      if(symbols[current_function_call]['param'][param_list[current_param_count-1]]['type'] == arg_type):
          quad = ['param', arg_name, None, symbols[current_function_call]['param'][param_list[current_argument_count-1]]['address']]
          quadruples.append(quad)
          current_argument_count += 1
      else:
          print("argument is not of expected type")
          sys.exit()
    else:
        print("Received more arguments than expected")
        sys.exit()
    #print("salida")
...
def p_pn_func_gosub(p):
    ...
    if (result_type != 'void'):
        result = nextAddress(result_type)
        symbols[current_function]['count'][result_type] += 1
        quad = ["=", symbols['global'][current_function_call]['address'],None,result]
        quadruples.append(quad)
        stack_operands.pop()
        stack_types.pop()
        stack_operators.pop()
